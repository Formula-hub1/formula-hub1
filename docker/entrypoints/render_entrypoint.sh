...
if [ "$FLASK_ENV" = "development" ] || [ "$FLASK_ENV" = "staging" ]; then
    echo "🚨 ENTORNO DEV/STAGE DETECTADO: Forzando re-sellado del historial de Alembic..."
    # 'base' indica que no hay migraciones aplicadas.
    flask db stamp base || echo "Advertencia: No se pudo sellar la BD a la base (puede estar vacía), continuando."
fi
