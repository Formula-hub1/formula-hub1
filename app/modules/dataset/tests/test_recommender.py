...
from core.services.DatasetRecommenderService import DatasetRecommenderService, SimilarityCalculator
...
class MockDataSet:
    ...
    def __init__(self, id, title, tags_str, author_ids, created_at, downloads_count=0):
        ...
        self.ds_meta_data = MockDSMetaData(tags=tags_str, authors=[MockAuthor(aid) for aid in author_ids])
...
class TestDatasetRecommenderService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.recommender = DatasetRecommenderService(
            dataset_repository=self.mock_dataset_repo, ds_download_repository=self.mock_download_repo
        )

        # 🎯 DATOS DE CANDIDATOS (Simulación de la BD)
        self.ds_target = MockDataSet(100, "Target DS", "web,js,api", [10], RECENT_DATE)

        self.candidate_A = MockDataSet(
            1, "API Tools", "web,api,http", [10, 11], RECENT_DATE, downloads_count=900
        )  # Alta popularidad, tags en común
        self.candidate_B = MockDataSet(
            2, "Old Docs", "manual", [20], OLD_DATE, downloads_count=100
        )  # Baja popularidad, antiguo
        self.candidate_C = MockDataSet(
            3, "UX Frontend", "ux,css", [10], RECENT_DATE, downloads_count=500
        )  # Media popularidad, autor en común
