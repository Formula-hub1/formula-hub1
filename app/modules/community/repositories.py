...
class DatasetCommunitySubmissionRepository(BaseRepository):
    ...
    def create_submission(self, dataset_id: int, community_id: int) -> Optional[DatasetCommunitySubmission]:
        ...
        submission = self.model(dataset_id=dataset_id, community_id=community_id, status=SubmissionStatus.PENDING)
...
class CommunityMembershipRepository(BaseRepository):
    ...
    def add_member(
        self, user_id: int, community_id: int, role: CommunityRole = CommunityRole.MEMBER
        ...
        membership = self.model(user_id=user_id, community_id=community_id, role=role)
