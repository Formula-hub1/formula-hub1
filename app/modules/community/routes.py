...
from app.modules.community.forms import (
    CommunityForm, 
    MemberForm, 
    SubmitDatasetForm, 
...
def add_member(slug):
    ...
    if form.validate_on_submit():
        membership = community_service.add_member(
            community.id,
            form.user_id.data,
            form.role.data
        )
...
def submissions(slug):
    ...
    return render_template('community/submissions.html',
                         community=community,
                         submissions=pending)
...
def review_submission(submission_id):
    ...
    if form.validate_on_submit():
        ...
        if action == 'approve':
            ...
            community_service.reject_submission(
                submission.id, 
                current_user.id, 
                form.feedback.data
            )
    ...
    return render_template('community/review_submission.html',
